 ...
 */
void handle_expose(
    Display *display, GC gc, GC rev_gc, XExposeEvent *expose_event,
    unsigned int win_width, unsigned int win_height,
    short pixels[1000][1000]) { /* if this is the first in a set of expose
                                   events - ignore this event. */
    ...
    XDrawPoint(display, expose_event->window, gc, win_width - 5,
               win_height - 5);
    ...
    XDrawArc(display, expose_event->window, gc, 50 - (30 / 2), 100 - (30 / 2),
             30, 30, 0, 360 * 64);
    {
        ...
        XDrawLines(display, expose_event->window, gc, points, npoints,
                   CoordModeOrigin);
    }
...
int main(int argc, char *argv[]) {
    ...
    /* make it no go out of bounds for full hd monitors */
    width = 1000;
    height = 1000;
    ...
    /* perform an events loop */
    {
        ...
        while (!done) {
            ...
            case Expose:
                /* redraw our window. */
                handle_expose(display, gc, rev_gc,
                              (XExposeEvent *)&an_event.xexpose, width, height,
                              pixels);
                break;
            ...
            case ButtonPress:
                ...
                handle_button_down(display, gc, rev_gc,
                                   (XButtonEvent *)&an_event.xbutton, width,
                                   height, pixels);
                break;
            case MotionNotify:
                /* invert the pixel under the mouse pointer. */
                handle_drag(display, gc, rev_gc,
                            (XButtonEvent *)&an_event.xbutton, width, height,
                            pixels);
                break;
            case KeyPress:
