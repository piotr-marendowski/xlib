...
int main(int argc, char *argv[]) {
    ...
    { /* this variable will contain the ID of the newly created pixmap. */
        ...
        /* variable. */
        int rc =
            XReadBitmapFile(display, win, "assets/icon.xbm", &bitmap_width,
                            &bitmap_height, &bitmap, &hotspot_x, &hotspot_y);
        /* check for failure or success. */
        switch (rc) {
        case BitmapOpenFailed:
            fprintf(stderr,
                    "XReadBitmapFile - could not open file 'icon.bmp'.\n");
            exit(1);
            break;
        ...
        { /* first, define forground and background colors for the cursor. */
            ...
            Status rc = XAllocNamedColor(display, screen_colormap, "black",
                                         &cursor_fg, &cursor_fg);
            if (rc == 0) {
                fprintf(stderr,
                        "XAllocNamedColor - canot allocate 'black' ??!!??\n");
                exit(1);
            }
            rc = XAllocNamedColor(display, screen_colormap, "white", &cursor_bg,
                                  &cursor_bg);
            if (rc == 0) {
                fprintf(stderr,
                        "XAllocNamedColor - canot allocate 'white' ??!!??\n");
                exit(1);
            }
